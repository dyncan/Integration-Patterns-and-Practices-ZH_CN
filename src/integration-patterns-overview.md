...
| 数据虚拟化                         | Salesforce实时地访问外部数据。这就不需要在Salesforce中持久化数据，然后在 Salesforce 和外部系统之间进行数据协调.                                                            |
...
- **虚拟集成**: 这个类别的模式解决了用户查看,搜索和修改存储在外部系统中的数据的需求.当你实施这种类型的集成的解决方案时,触发的应用程序必须调用其他应用程序,并与他们的数据进行实时交互.这种类型的集成消除了跨系统数据复制的需要,也意味着用户总是与最新的数据进行交互.
